...
question = "What is llm agent memory?"
...
system = """You are an assistant for question-answering tasks.
    ...
    and write source of information you've refered while making answer (title, URL)"""
...
class GraphState(TypedDict):
    ...
    question: str
    generation: str
    terminate_by_relevance_failure: str
    was_hallucinated: str
    web_search_completed: str
    web_search: str
    documents: List[str]
...
def websearch_result_grade_documents(state):
    """
    Determines whether the retrieved documents are relevant to the question
    If any document is not relevant, we will set a flag to run web search
    ...
    print("---CHECK websearch_result_grade_documents RELEVANCE TO QUESTION---")
    question = state["question"]
    documents = state["documents"]
    web_search_completed = state["web_search_completed"]
    ...
    for d in documents:
        ...
        # Document not relevant
            ...
            # We set a flag to indicate that we want to run web search
            continue
    return {"documents": filtered_docs, "question": question, "websearch": web_search, "web_search_completed": web_search_completed}
...
def web_search(state):
    ...
    state["web_search_completed"] = "Yes"

    return {"web_search_completed": "Yes", "documents": documents, "question": question}
...
def decide_to_generate(state):
    ...
    if web_search_completed == "Yes" and web_search == "Yes":
        return "terminate_by_relevance_fail"
...
### Conditional edge
def decide_to_relevance_fail(state):
    print("---ASSESS relevance terminal required ---")
    state["question"]
    web_search = state["web_search"]
    state["documents"]
    ...
    if web_search == "Yes":
        ...
        return "terminate_by_relevance_failure"
    else:
        # We have relevant documents, so generate answer
        print("---DECISION: GENERATE---")
        return "generate"
...
def grade_generation_v_documents_and_question(state):
    ...
    was_hallucinated = state["was_hallucinated"]
    ...
    if grade == "yes":
        ...
        if was_hallucinated == 'Yes':
            return 'terminate_by_hallucination'
        else:
            state["was_hallucinated"] = 'Yes'
            return "not supported"


def failed_to_relevance_check(state):
    print("--- Document 의 Relevant 체크 실패 ---")
    print("failed: not relevant")
    return

def failed_to_hallucination(state):
    print("--- Hallucination 체크 실패 ---")
...
workflow.add_node("failed_to_relevance_check", failed_to_relevance_check)
workflow.add_node("failed_to_hallucination", failed_to_hallucination)
...
workflow.add_conditional_edges(
    ...
    decide_to_generate,
    {
        "terminate_by_relevance_fail": "failed_to_relevance_check",
        "websearch": "websearch",
        "generate": "generate",
    },
)
...
workflow.add_conditional_edges(
    ...
    grade_generation_v_documents_and_question,
    {
        ...
        "terminate_by_hallucination": "failed_to_hallucination"
    },
)
...
inputs = {"question": "who is elon musk?", "web_search_completed": "No", "was_hallucinated": "No"}
